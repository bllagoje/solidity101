...
contract TimeLock {
    // Time lock
    error NotOwnerError();
    error AleradyQueuedError(bytes32 txId);
    ...
    uint public constant MIN_DELAY = 10; // 10 sec
    uint public constant MAX_DELAY = 1000; // 1000 sec
    uint public constant GRACE_PERIOD = 1000; // 1000 sec
    ...
    function queue(address _target, uint _value, string calldata _func, bytes calldata _data, uint _timestamp) external onlyOwner {
        ...
        if (queued[txId]) {
            revert AleradyQueuedError(txId);
        }
